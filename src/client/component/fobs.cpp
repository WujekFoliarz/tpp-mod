...
namespace fobs
{
	namespace
	{
		...
		vars::var_ptr var_alive_fobs_server_http_url;
		...
		bool alive_fobs_enabled()
		{
			return var_fob_security_challenge_mode->current.get_int() == 2;
		}
			...
			state.access([&](state_t &s)
					...
					{				
						...
						for (auto i = 0; i < count; i++)
						{
							...
							int mb_num = 0;
							for (const auto& motherbase : curPlayer["mother_base_param"])
							{
								fob_target->playerInfos[i].mother_base_id[mb_num] = motherbase["mother_base_id"];
								fob_target->playerInfos[i].area_id[mb_num] = motherbase["area_id"].get<char>();
								fob_target->playerInfos[i].construct_param2[mb_num] = static_cast<char>((motherbase["construct_param"].get<int>() >> 22) & 0x3F);
								fob_target->playerInfos[i].security_rank[mb_num] = motherbase["security_rank"].get<char>();
								fob_target->playerInfos[i].platform_count[mb_num] = motherbase["platform_count"].get<char>();
								mb_num++;
							}
							fob_target->playerInfos[i].mother_base_num = static_cast<char>(mb_num) + 1;

							fob_target->playerInfos[i].owner_ugc = 1;
							fob_target->playerInfos[i].league_rank_grade = 0;
							fob_target->playerInfos[i].league_rank_rank = 0;
							fob_target->playerInfos[i].sneak_rank_grade = curPlayer["ranking"]["fob_grade"].get<char>();
							fob_target->playerInfos[i].sneak_rank_rank = curPlayer["ranking"]["fob_rank"];
							fob_target->playerInfos[i].espionage_score = curPlayer["ranking"]["fob_point"];
							fob_target->playerInfos[i].espionage_win = 0;
							fob_target->playerInfos[i].espionage_total = 0;

							for (auto o = 0; o < 10; o++)
							{
								fob_target->playerInfos[i].staff_count[o] = 0;
								fob_target->playerInfos[i].staff_num += 0;
							}

							fob_target->playerInfos[i].usable_resource.fuel_resource = 0;
							fob_target->playerInfos[i].usable_resource.biotic_resource = 0;
							fob_target->playerInfos[i].usable_resource.common_metal = 0;
							fob_target->playerInfos[i].usable_resource.minor_metal = 0;
							fob_target->playerInfos[i].usable_resource.precious_metal = 0;

							fob_target->playerInfos[i].nameplate_id = static_cast<char>(1);
	...
	public:
		...
		void start() override
		{
			...
			cmd_get_fob_target_list_result_unpack_hook.create(SELECT_VALUE_LANG(0x140817CB0, 0x0), cmd_get_fob_target_list_result_unpack_stub);
			cmd_get_fob_target_list_option_pack_hook.create(SELECT_VALUE_LANG(0x140817B60, 0x0), cmd_get_fob_target_list_option_pack_stub);

			cmd_set_security_challenge_option_pack_hook.create(SELECT_VALUE_LANG(0x140858A70, 0x0), cmd_set_security_challenge_option_pack_stub);
			cmd_set_security_challenge_result_unpack_hook.create(SELECT_VALUE_LANG(0x140849F50, 0x0), cmd_set_security_challenge_result_unpack_stub);

			cmd_get_playerlist_result_unpack_hook.create(SELECT_VALUE_LANG(0x1407E3350, 0x0), cmd_get_playerlist_result_unpack_stub);
			cmd_set_currentplayer_result_unpack_hook.create(SELECT_VALUE_LANG(0x1407E3B50, 0x0), cmd_set_currentplayer_result_unpack_stub);
			cmd_sync_mother_base_option_pack_hook.create(SELECT_VALUE_LANG(0x14080F7B0, 0x0), cmd_sync_mother_base_option_pack_stub);
			cmd_get_own_fob_list_result_unpack_hook.create(SELECT_VALUE_LANG(0x140846BD0, 0x0), cmd_get_own_fob_list_result_unpack_stub);
			cmd_sync_soldier_bin_pack_hook.create(SELECT_VALUE_LANG(0x1408132A0, 0x0), cmd_sync_soldier_bin_pack_stub);
			cmd_sync_resource_result_unpack_hook.create(SELECT_VALUE_LANG(0x140812AA0, 0x0), cmd_sync_resource_result_unpack_stub);

			fob_target_receive_enemy_basic_info_hook.create(SELECT_VALUE_LANG(0x1407EF730, 0x0), fob_target_receive_enemy_basic_info_stub);

			fob_mission2_callback_update_hook.create(SELECT_VALUE_LANG(0x141694330, 0x0), fob_mission2_callback_update_stub);
